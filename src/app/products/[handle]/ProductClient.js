                    ...
                    <div className="flex flex-col-reverse h-[50%] md:flex-row gap-4 lg:w-3/5">
                        <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-y-auto no-scrollbar md:w-20 shrink-0">
                            {media.map((item, idx) => (
                                ...
                                >
                                    <img
                                        src={item.type === 'model' ? (media.find(m => m.type !== 'model')?.url || item.previewUrl || "https://placehold.co/100x100?text=3D") : item.url}
                                        className="w-full h-full object-cover bg-white"
                                        alt={item.altText || "thumb"}
                                    />
                                    {item.type === 'model' && (
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <div className="bg-white p-1 px-2.5 rounded-full text-stone-900 shadow-[0_2px_10px_rgba(0,0,0,0.1)] border border-gray-100">
                        ...
                        <div className="relative inline-block bg-[#F9F8F6] h-[50%] overflow-hidden group w-full">
                            ...
                            <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                <button onClick={() => setActiveImg(prev => prev === 0 ? media.length - 1 : prev - 1)} className="p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm hover:bg-white pointer-events-auto">
                                    <ArrowLeft size={16} />
                                </button>
                                <button onClick={() => setActiveImg(prev => (prev + 1) % media.length)} className="p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm hover:bg-white pointer-events-auto">
                                    <ArrowRight size={16} />
                                </button>
                            </div>
                        </div>
