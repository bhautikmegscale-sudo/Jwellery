...
export default function OrderConfirmationPage() {
    ...
    useEffect(() => {
        ...
        if (storedOrder) {
            ...
            // Only clear after user navigates away
        } else {
            ...
            <div className="min-h-screen bg-[#faf9f6] py-24 px-4 sm:px-6 lg:px-8 font-sans">
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="text-center mb-12">
                        ...
                        <h1 className="text-4xl font-serif text-[#1a1a1a] mb-2">Thank you for your order</h1>
                        <p className="text-gray-600">We've received your order and are preparing it with care.</p>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 border-b border-gray-100 bg-gray-50/50">
                            <div>
                                ...
                                <p className="font-medium text-[#1a1a1a]">{orderData.orderNumber}</p>
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-widest text-gray-500 mb-1">Date</p>
                                <p className="font-medium text-[#1a1a1a]">{formatDate(orderData.date)}</p>
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-widest text-gray-500 mb-1">Total</p>
                                <p className="font-medium text-[#1a1a1a]">₹{orderData.total.toFixed(2)}</p>
                            </div>
                            <div>
                        ...
                        <div className="p-6">
                            ...
                            <div className="mt-6 pt-6 border-t border-gray-100 space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-[#1a1a1a]">₹{orderData.subtotal.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-600">Tax</span>
                                    <span className="text-[#1a1a1a]">₹{orderData.tax.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-base font-medium pt-2 border-t border-gray-100">
                                    <span className="text-[#1a1a1a]">Total</span>
                                    <span className="text-[#1a1a1a]">₹{orderData.total.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400">{getEstimatedDelivery()}</p>
                                    </div>
                                </div>
                                <div className="text-sm text-gray-400">
                                    <p className="font-medium text-white mb-1">Shipping Address</p>
                                    <p>{orderData.customer.firstName} {orderData.customer.lastName}</p>
                                    <p>{orderData.customer.address1}</p>
                                    <p>{orderData.customer.city}, {orderData.customer.zip}</p>
                                </div>
                            </div>
                        </div>
                    </div>
